...
def list_teams(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_user_teams(db, skip=skip, limit=limit)

@router.post("/teams", response_model=schemas.UserTeamBase)
def create_team(team: schemas.UserTeamCreate, db: Session = Depends(get_db)):
    return crud.create_user_team(db, name=team.name, format_id=team.format_id)

@router.get("/teams/{team_id}", response_model=schemas.UserTeamBase)
def get_team(team_id: int, db: Session = Depends(get_db)):
    db_team = crud.get_user_team(db, team_id)
    if not db_team:
        raise HTTPException(status_code=404, detail="Team not found")
    return db_team

@router.put("/teams/{team_id}", response_model=schemas.UserTeamBase)
def update_team(team_id: int, team: schemas.UserTeamUpdate, db: Session = Depends(get_db)):
    db_team = crud.update_user_team(db, team_id, name=team.name, format_id=team.format_id)
    if not db_team:
        raise HTTPException(status_code=404, detail="Team not found")
    return db_team
...
def delete_team(team_id: int, db: Session = Depends(get_db)):
    success = crud.delete_user_team(db, team_id)
    if not success:
        raise HTTPException(status_code=404, detail="Team not found")
...
def update_pokemon(pokemon_id: int, updates: schemas.UserTeamPokemonUpdate, db: Session = Depends(get_db)):
    db_pokemon = crud.update_user_team_pokemon(db, pokemon_id, updates.dict(exclude_unset=True))
    if not db_pokemon:
        raise HTTPException(status_code=404, detail="Pokemon slot not found")
    return db_pokemon

@router.put("/pokemon/{pokemon_id}/moves/{slot}", response_model=schemas.UserTeamPokemonMoveBase)
def update_pokemon_move(pokemon_id: int, slot: int, move: schemas.UserTeamPokemonMoveUpdate, db: Session = Depends(get_db)):
    db_move = crud.update_user_team_pokemon_move(db, pokemon_id, slot, move_id=move.move_id)
    if not db_move:
        raise HTTPException(status_code=404, detail="Move slot not found")
    return db_move
